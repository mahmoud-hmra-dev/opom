...
function updateTaskUrls(projectId: string, tasks: IProjectDigestTask[]) {
  ...
  for (const task of tasks) {
    task.url = `${baseUrl}/opom/projects/${projectId}?tab=tasks-list&task=${task.id}`;
  }
}
...
function updateMetadata(project: IProjectDigest, subscriberName: string) {
  ...
  project.settings_url = `${getBaseUrl()}/opom/settings/notifications`;
  project.project_url = `${getBaseUrl()}/opom/projects/${project.id}?tab=tasks-list`;
}
